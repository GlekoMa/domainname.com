...
这里另外介绍第二种爬取方法：使用rvest包。可以直接爬取出正确的表。
```
library(rvest)
#假设文件被保存为bb.html
theURL <- "bb.html"
presidents2 <- (read_html(theURL) %>% 
    ...
    html_table())[[4]]
#read_html() 爬取页面源代码
#html_elements() 提取网页中指定元素的部分
#html_table() 根据源代码生成数据框或列表（就此题目来说，若上一步处理了使用html_elements函数，则生成列表；若上一步使用了html_element函数，则生成数据框。详请自行查看函数用法。）
```
